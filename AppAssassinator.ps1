...
##Apps to remove - NOTE: Some applications like Chrome and WPS Office seem to have an unusual uninstall so needed to sort on their own
...
foreach ($app in $allstring) {
    ...
    foreach ($blacklistedApp in $blacklistapps) {
        ...
        elseif ($displayName -like $blacklistedApp -and $uninstallString -like "*.exe*") {
            $uninstallCommand = "$uninstallString /quiet --uninstall --runimmediately /S"
            Write-Host "Uninstalling $displayName"
            Start-Process "cmd.exe" -ArgumentList "/c $uninstallCommand"
            break
        }
...
# Remove WPS Office (all User Profile)
